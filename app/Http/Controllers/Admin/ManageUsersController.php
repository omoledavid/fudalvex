...
class ManageUsersController extends Controller
{
    ...
    public function clearacct(Request $request, $id)
    {
        ...
        $tp_trans = Tp_Transaction::where('user', $id)->get();
        if (!empty($tp_trans)) {
            foreach ($tp_trans as $tp_trans) {
                Tp_Transaction::where('id', $tp_trans->id)->delete();
            }
        }
        $swapRecords = CryptoRecord::where('user_id', $id)->get();
        if (!empty($swapRecords)) {
            foreach ($swapRecords as $swapRecords) {
                CryptoRecord::where('id', $swapRecords->id)->delete();
            }
        }
